...
Browser emulation is reproducible visual QA, not an iPhone Safari real-device pass. Browser CI does not provide evidence of actual focus loss/return or a `document.hidden` transition. The Chapter 1 real-device Safari pass remains **required-pending** until actually performed; never relabel Chromium/WebKit emulation as that evidence.
...
8. Lifecycle: pause, resume, browser focus loss/return, document hide/show, duplicate input, close/“あとで続ける”, repeated open, reduced motion, console errors, failed requests, memory/texture release, and focus restoration. Browser CI covers explicit UI pause/resume; Node tests own the `blur` and `document.hidden` / `visibilitychange` contracts. Actual focus loss/return and document hide/show require the real-iPhone Safari pass.
...
For a real iPhone Safari pass, repeat the critical story path with touch input and safe areas, observe frame pacing/memory during scene changes and battle handoff, and verify actual focus loss/return plus a foreground→background→foreground transition: the document becomes hidden then visible, the Story clock does not advance while hidden, and hold/pause/resume remain reliable after return. Until that hardware or device-farm pass is recorded, report it as **required-pending** and do not merge or publish the Chapter 1 playable vertical slice without the recorded pass or an explicit owner-approved risk exception.
