...
Browser emulation is reproducible visual QA, not an iPhone Safari real-device pass. In particular, a Playwright focus loss/return is not evidence of an actual `document.hidden` transition. The Chapter 1 real-device Safari pass remains **required-pending** until actually performed; never relabel Chromium/WebKit emulation as that evidence.
...
8. Lifecycle: pause, resume, browser focus loss/return, document hide/show, duplicate input, close/“あとで続ける”, repeated open, reduced motion, console errors, failed requests, memory/texture release, and focus restoration. Browser CI may satisfy the pause/resume and focus-loss portions; actual document hide/show requires the real-iPhone Safari pass.
