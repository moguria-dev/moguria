...
For Chapter 1, additionally verify all four motions at `390×844` and `375×667`, normal and reduced motion, before/after pause, after browser focus loss/return, after actual document hide/resume, and at the earliest and delayed fragment hold commitment. Runtime video and a pivot-overlay inspection remain required release evidence; do not infer them from a manifest-only test. Browser CI records the production focus-loss/return and explicit pause/resume paths, while the Node Story-player tests own the `document.hidden` / `visibilitychange` clock contract. Neither substitutes for the required real-iPhone Safari background hide/show check before publishing the Chapter 1 playable vertical slice, unless the owner explicitly approves and records a risk exception.
