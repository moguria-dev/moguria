...
For Chapter 1, additionally verify all four motions at `390×844` and `375×667`, normal and reduced motion, before/after explicit UI pause, and at the earliest and delayed fragment hold commitment. Runtime video and a pivot-overlay inspection remain required release evidence; do not infer them from a manifest-only test. Browser CI records the normal/reduced continuous videos, explicit UI pause/resume, and early/delayed holds; it does not claim actual browser focus loss/return. The Node Story-player tests own the production `blur` and `document.hidden` / `visibilitychange` clock contracts. Actual focus loss/return and background hide/show require real-iPhone Safari evidence. Do not merge or publish the Chapter 1 playable vertical slice without that recorded device pass or an explicit owner-approved, recorded risk exception.
